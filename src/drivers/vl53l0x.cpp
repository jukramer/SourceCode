...
void VL53L0X::writeReg(uint8_t reg, uint8_t value)
{
    ...
    i2c_write_blocking(I2C_CHANNEL_TOF, address, buffer, 2, false);
}
...
void VL53L0X::writeReg16Bit(uint8_t reg, uint16_t value)
{
    ...
    i2c_write_blocking(I2C_CHANNEL_TOF, address, buffer, 3, false);
}
...
void VL53L0X::writeReg32Bit(uint8_t reg, uint32_t value)
{
    ...
    i2c_write_blocking(I2C_CHANNEL_TOF, address, buffer, 5, false);
}

uint8_t VL53L0X::readReg(uint8_t reg)
{
    uint8_t value = 0;
    printf("Reading reg 0x%02X from address 0x%02X\n", reg, address);
    int res = i2c_write_blocking(I2C_CHANNEL_TOF, address, &reg, 1, true); // true to keep master control of bus
    printf("I2C write status: %d\n", res);
    res = i2c_read_blocking(I2C_CHANNEL_TOF, address, &value, 1, false);
    printf("I2C read status: %d, value: 0x%02X\n", res, value);
    return value;
}
uint16_t VL53L0X::readReg16Bit(uint8_t reg)
{
    uint8_t buffer[2] = {0, 0};
    i2c_write_blocking(I2C_CHANNEL_TOF, address, &reg, 1, true);   // true to keep master control of bus
    i2c_read_blocking(I2C_CHANNEL_TOF, address, buffer, 2, false); // false to release the bus after read
...
uint32_t VL53L0X::readReg32Bit(uint8_t reg)
{
    ...
    i2c_write_blocking(I2C_CHANNEL_TOF, address, &reg, 1, true);   // true to keep master control of bus
    i2c_read_blocking(I2C_CHANNEL_TOF, address, buffer, 4, false); // false to release the bus after read
...
void VL53L0X::writeMulti(uint8_t reg, uint8_t const *src, uint8_t count)
{
    ...
    last_status = i2c_write_blocking(I2C_CHANNEL_TOF, address, buffer, count + 1, false);
}

void VL53L0X::readMulti(uint8_t reg, uint8_t *dst, uint8_t count)
{
    // First write the register address we want to read from
    last_status = i2c_write_blocking(I2C_CHANNEL_TOF, address, &reg, 1, true);
    // Now read count bytes into the dst buffer
    if (last_status == PICO_ERROR_NONE)
    {
        last_status = i2c_read_blocking(I2C_CHANNEL_TOF, address, dst, count, false);
    }
}
