...
VL53L1X::VL53L1X(i2c_inst_t *i2c_port, uint8_t i2c_address, uint16_t io_timeout_ms)
    : _i2c_port(i2c_port), address(i2c_address), io_timeout(io_timeout_ms), did_timeout(false), calibrated(false), saved_vhv_init(0), saved_vhv_timeout(0), distance_mode(Unknown)
{
}
...
{
    printf("VL53L1X init, device address: %02X\n", address);
